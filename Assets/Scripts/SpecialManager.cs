...
public class SpecialManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            AddSpecial("Beast's Sword", 1, 1, null, null);
        }
    }
    ...
    public void AddSpecial(string specialName, int specialType, float specialTime, Sprite specialImage, string specialDescription)
    {
        for (int i = 0; i < specialSlots.Length; i++)
        {
            if (!specialSlots[i].isFull)
            {
                specialSlots[i].AddSpecial(specialName, specialType, specialTime, specialImage, specialDescription);
                return;
            }
        }
